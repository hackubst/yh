<?php
//门店管理员后台
class DcpAction extends GlobalAction {
	//待处理数据
	...
	function _initialize() {
		...
		if ($this->login_user['role_type'] != 6) redirect(U('/logout')); //已登录但不是管理员，则强制退出要求重新登录
		...
		if (!in_array(ACTION_NAME, array('index', 'set_menu', 'uploadHandler', 'delImage', 'common_article')) && $user_id != 1)
		{
            ...
            if (stristr(MODULE_NAME, 'ajax') || in_array(MODULE_NAME, array('DcpIntegral'))) {
                ;
            }else if (!$this->checkpriv('/' . MODULE_NAME . '/' . ACTION_NAME, true))
			{
				redirect(U('/dcp'), 5, '您没有访问此操作的权限，请联系系统管理员！');
			}
		}
	...
	public function index(){
		if (!$this->checkpriv('/DcpOrder/get_pre_deliver_order_list', false))
		{
        ...
        log_file(0000);
		redirect(U('/DcpOrder/get_pre_deliver_order_list'));
	}
	...
	*/
	protected function get_all_my_priv() {
		include_once(CONF_PATH . 'dept_priv.php');
