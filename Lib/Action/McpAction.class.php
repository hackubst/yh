...
class McpAction extends GlobalAction {
	...
	function _initialize() {
		...
		if (!$this->login_user)
		    exit; //没登录先进登录页面
		if ($this->login_user['role_type'] != 4) redirect(U('/logout')); //已登录但不是管理员，则强制退出要求重新登录
	...
	{
		if (!$this->checkpriv('/McpConfig/base_config', false))
		{
		...
		redirect(U('/McpConfig/base_config'));
	}
	...
	*/
	protected function get_all_my_priv() {
		...
		if ($this->login_user['role_type'] == 4) { //超级管理员有所有权限
