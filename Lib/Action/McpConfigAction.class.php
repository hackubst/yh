...
class McpConfigAction extends McpAction
{
    ...
    public function base_config()
    {
        ...
        if ($act == 'save') //提交保存时
        {
            ...
            $ConfigBaseModel = new ConfigBaseModel();
            $ConfigBaseModel->setConfigs($data);
            $this->success('恭喜你，参数设置成功了!', '/McpConfig/base_config/mod_id/0');
        }
        //用户登录大图
...
//        <td>{$this.recharge}</td>
//        <td>{$this.card}</td>
//        <td>{$this.recharge_profit}</td>
//        <td>{$this.card_profit}</td>
//        <td>{$this.today_profit}</td>
        ...
        if($time_data['start_time'])
        {
            ...
            $today = $time_data['end_time'];
            $dcp_write_off_rate = $GLOBALS['config_info']['DCP_WRITE_OFF_RATE'];
            for ($i = 1; $i <= $count; $i++) {
                $start_time = $today - $i * 24 * 60 * 60;
                $end_time = $start_time + 24 * 60 * 60;
                $data[$i]['addtime'] = date('Y-m-d', $start_time);
                //充值
                $recharge = $account_obj->where('state = 0 AND addtime > ' . $start_time . ' AND addtime < ' . $end_time . ' AND change_type ='
                    . AccountModel::RECHARGE . ' AND operater =' . $admin_id)->getField('sum(amount_in)');
                $recharge_out = $account_obj->where('state = 0 AND addtime > ' . $start_time . ' AND addtime < ' . $end_time . ' AND change_type ='
                    . AccountModel::RECHARGEOUT . ' AND user_id =' . $admin_id)->getField('sum(amount_out)');
                $data[$i]['recharge_profit'] = $recharge - $recharge_out;
                $data[$i]['recharge'] = $recharge / 1000;
                //卡密
                $card = $account_obj->where('addtime > ' . $start_time . ' AND addtime < ' . $end_time . ' AND change_type ='
                    . AccountModel::CARD_RETURN . ' AND operater =' . $admin_id)->getField('sum(amount_in)');
                $data[$i]['card'] = $card / 1000;
                $data[$i]['card_profit'] = $card * $dcp_write_off_rate / 100;

                $data[$i]['today_profit'] =( $data[$i]['recharge_profit'] + $data[$i]['card_profit'] ) / 1000;
            }
        }
        else{
            //统计查询
        ...
        $today_profit = array_sum(array_column($data,'today_profit')) ? : 0;
        $card_profit = array_sum(array_column($data,'card_profit')) ? : 0;
        $recharge_profit = array_sum(array_column($data,'recharge_profit')) ? : 0;
        $recharge = array_sum(array_column($data,'recharge')) ? : 0;
        $card = array_sum(array_column($data,'card')) ? : 0;
        ...
        if($time_data['start_time'])
        {
            ...
            $p = new Page($count, 10, '', $page);//3为每页显示的条数,$page为传过来的页码
            $pa = ($page - 1) * 10;//计算每页的数据从第几条开始
            $data = array_slice($data, $pa, 10);//重组数组
            $page = $p->show();//传递页码到前端显示
            $this->assign('page', $page);   //传递页码
        }
        $this->assign('today_profit',$today_profit);
        $this->assign('card_profit',$card_profit);
        $this->assign('recharge_profit',$recharge_profit);
