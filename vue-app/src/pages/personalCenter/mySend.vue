        ...
        <mescroll-vue ref="mescroll" :down="mescrollDown" :up="mescrollUp" @init="mescrollInit">
            <ul class="relief_list">
                <li class="relief_info" :class="{dual: index%2 != 0}" v-for="(i, index) in relief_list" :key="index">
                    <div>{{i.total_money | changeBigNum}}</div>
                    <div>{{i.num}}</div>
                    <div>{{i.residue_num}}</div>
                    <div>{{i.addtime | formatDateYearDot}}</div>
                    <!-- <div @click="cancle(i.red_packet_id)">{{i.is_cancel==0?'撤销':'已撤销'}}</div> -->
                </li>
            </ul>
        </mescroll-vue>
    </div>
    ...
    methods: {
        ...
        get_list(firstRow, fetchNum) {
            this.$Api({
                ...
                if (!err) {
                    let arr = data.data.red_packet_list
			        // 如果是第一页需手动制空列表
