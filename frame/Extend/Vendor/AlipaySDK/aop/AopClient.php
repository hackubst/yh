...
class AopClient {
	...
	*/
	...
	function verify($data, $sign, $rsaPublicKeyFilePath, $signType = 'RSA') {
		log_file('signType ='.$signType .', data ='.$data .' , rsaPublicKeyFilePath ='.$rsaPublicKeyFilePath .' , sign ='.$sign. ', alipayrsaPublicKey='.$this->alipayrsaPublicKey , 'signType');
		...
		if ("RSA2" == $signType) {
			...
			log_file('openssl_error_string:'.openssl_error_string().'------------------- res ='.$res,'openssl_error_string');
		} else {
	...
	public function checkResponseSign($request, $signData, $resp, $respObject) {
		...
		if (!$this->checkEmpty($this->alipayPublicKey) || !$this->checkEmpty($this->alipayrsaPublicKey)) {
			...
			if (!$this->checkEmpty($responseSubCode) || ($this->checkEmpty($responseSubCode) && !$this->checkEmpty($signData->sign))) {
				...
				if (!$checkResult) {
					log_file($signData->signSourceData, 'signSourceData');
