...
//------------------------------------------------------用户体系
    ...
    /* 注册规则
	   在注册环信账户的时候，需要注意环信 ID 的规则：
		...
		不区分大小写。系统忽略大小写，认为 AA、Aa、aa、aA 都是一样的。
		如果系统已经存在了环信 ID 为 AA 的用户，再试图使用 aa 作为环信 ID 注册新用户，系统返回用户名重复，
	    以此类推。但是请注意：环信 ID 在数据上的表现形式还是用户最初注册的形式，注册时候使用的大写就保存大写，
	    是小写就保存小写。即：使用 AA 注册，环信保存的 ID 就是 AA；使用 Aa 注册，环信保存的 ID 就是 Aa，以此类推。
	*/
