...
//    print_r($ip);
    ...
    $appkey = "a8fcfe62bae5e32344b6484c9bc1e055";
//************1.根据IP/域名查询地址************
    ...
    if ($result) {
        if ($result['error_code'] == '0') {
            ...
            $row_province = $Province->where('province_name = "' . $result['result']['Province'] . '"')->limit(0, 1)->select();
            //如果是直辖市 只返回省份ID，城市ID为0
            if ($result['result']['City'] == '上海市' || $result['result']['City'] == '北京市' || $result['result']['City'] == '天津市' || $result['result']['City'] == '重庆市')
            {
                return array('province_id' => $row_province[0]['province_id'], 'city_id' => 0, 'province_name' => $result['result']['Province'], 'city_name' => $result['result']['City']);
            }

            //否则根据城市名称查询城市ID并返回 省份ID和城市ID
            $row_city = $City->where('city_name = "' . $result['result']['City'] . '"')->limit(0, 1)->select();
            return array('province_id' => $row_province[0]['province_id'], 'city_id' => $row_city[0]['city_id'], 'province_name' => $result['result']['Province'], 'city_name' => $result['result']['City']);
        }
    }
    return array();
